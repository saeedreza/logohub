...
async function testAPI(baseUrl = 'http://localhost:3000') {
  ...
  try {
    ...
    console.log(
      `   🎯 First 5: ${logos.data.logos.map(l => l.name).join(', ')}\n`
    );
    ...
    const pngResponse = await axios.get(
      `${baseUrl}/api/v1/logos/${logoId}?format=png&size=64`,
      {
        responseType: 'arraybuffer',
      }
    );
    console.log(
      `   ✅ ${logoId} PNG (64px): ${pngResponse.data.length} bytes\n`
    );

    // Test metadata
    console.log('5. Testing metadata...');
    const metadata = await axios.get(
      `${baseUrl}/api/v1/logos/${logoId}/metadata`
    );
    ...
    const corsResponse = await axios.get(
      `${baseUrl}/api/v1/logos/${logoId}?size=64`,
      {
        headers: {
          Origin: baseUrl.includes('localhost')
            ? 'http://localhost:5173'
            : 'https://logohub.dev',
        },
        responseType: 'arraybuffer',
    ...
    console.log(
      `   ✅ CORS headers: ${corsHeaders ? 'Configured' : 'Missing'}\n`
    );
